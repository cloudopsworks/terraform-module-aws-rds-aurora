...
resource "aws_kms_key" "this" {
  count                   = try(var.settings.storage.encryption.enabled, false) && (try(var.settings.storage.encryption.kms_key_id, "") == "" || try(var.settings.storage.encryption.kms_key_alias, "") == "") ? 1 : 0
